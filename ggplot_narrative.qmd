...
This graphic is constructed from several *additive* components[^ggplot_narrative-1] including:

[^ggplot_narrative-1]: Literally, we add these toghter using the plus '+' sign just like we were going to develop an equation.
...
Here is an example, where I have the color of the shapes determined by a value in the `data.frame` but have the shape[^ggplot_narrative-2] applied to all the points, independent of any data in the `data.frame`.

[^ggplot_narrative-2]: The shapes are the same as the `pch` offerings covered in the lecture on graphing using Base R routines [here](https://dyerlab.github.io/ENVS-Lectures/visualization/basic_visualization/narriative.nb.html).
