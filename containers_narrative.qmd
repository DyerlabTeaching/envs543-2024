...
Vectors are the most basic data container in `R`. They *must* contain data of the exact same type and are constructed using the `combine()` function, which is abbreviated as `c()` because good programmers are lazy programmers. [^containers_narrative-1]

[^containers_narrative-1]: The more lines of code that you write, the more likely there will be either a grammatical error (easier to find) or a logical one (harder to find).
